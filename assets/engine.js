(() => {
  // engine-src/ws.stub.js
  var WebSocket = class {
  };

  // engine-src/figma-patch.js
  function getCdpPort() {
    return 9222;
  }

  // engine-src/figma-client.js
  var SEMANTIC_VAR_DEFAULTS = {
  ...
  var FigmaClient = class {
    ...
    static async listPages() {
      ...
      return pages.filter((p) => p.url && p.url.includes("figma.com")).map((p) => ({ title: p.title, id: p.id, url: p.url, wsUrl: p.webSocketDebuggerUrl }));
    }
    /**
     ...
     */
    async connect(pageTitle = null, { timeoutMs = 15e3 } = {}) {
      const port = getCdpPort();
      const response = await fetch(`http://localhost:${port}/json`);
      const pages = await response.json();
      const isDesignPage = (p) => p.url && /figma\.com\/(design|file)\//.test(p.url);
      let page;
      if (pageTitle) {
    ...
    async parseJSXBatch(jsxArray, options = {}) {
      ...
      const allFontMap = /* @__PURE__ */ new Map();
      const allFonts = [];
      ...
      parsed.forEach(({ props, children }) => {
        ...
        collected.fonts.forEach((f) => {
          ...
          if (!allFontMap.has(key)) {
            allFontMap.set(key, f);
            allFonts.push(f);
          }
        });
    ...
    ` : "";
      const framesCodes = parsed.map(({ props, children }, frameIdx) => {
        ...
        const isNumeric = (v) => v !== void 0 && v !== "fill" && v !== "hug";
        const rawW = isNumeric(props.w) ? props.w : isNumeric(props.width) ? props.width : void 0;
        const rawH = isNumeric(props.h) ? props.h : isNumeric(props.height) ? props.height : void 0;
        const hasExplicitWidth = rawW !== void 0;
        const width = rawW !== void 0 ? rawW : 320;
        const hasExplicitHeight = rawH !== void 0;
        const height = rawH !== void 0 ? rawH : 200;
        const bg = props.bg || props.fill || null;
        const stroke = props.stroke || null;
        ...
        const hug = props.hug || "";
        const opacity = props.opacity !== void 0 ? Number(props.opacity) : null;
    ...
    async prefetchIconSvgs(children) {
      const iconNames2 = this.collectIconNames(children);
      if (iconNames2.size === 0) return {};
      const svgMap = {};
      const fetches = [...iconNames2].map(async (iconName) => {
        try {
    ...
    validateJsxProps(jsx) {
      ...
      const levenshtein = (a, b) => {
        ...
        const d = Array.from({ length: m2 + 1 }, (_, i) => [i, ...Array(n).fill(0)]);
    ...
    collectFontsAndVarUsage(items) {
      ...
      let usesVars = false;
      const check = (v) => {
        if (this.isVarRef(v)) usesVars = true;
      };
      const walk = (list) => {
        list.forEach((item) => {
          if (item._type === "text") {
    ...
    generateChildrenCode(items, parentVar, parentFlex, ctx) {
      return items.map((item) => {
        ...
        if (item._type === "text") {
          ...
          const tLineHeight = item.lineHeight !== void 0 ? dimUnit(item.lineHeight) : null;
          const tLetterSpacing = item.letterSpacing !== void 0 ? dimUnit(item.letterSpacing) : null;
          const tTruncate = item.truncate === true || item.truncate === "true";
          const tMaxLines = item.maxLines !== void 0 ? parseInt(item.maxLines) : null;
        ...
        ${tMaxLines !== null ? `try { el${idx}.maxLines = ${tMaxLines}; } catch(e) {}` : ""}`;
          ...
          const fP = item.p !== void 0 ? item.p : item.padding !== void 0 ? item.padding : null;
          const fPx = item.px !== void 0 ? item.px : fP !== null ? fP : 0;
          const fPy = item.py !== void 0 ? item.py : fP !== null ? fP : 0;
          const fPt = item.pt !== void 0 ? Number(item.pt) : Number(fPy);
          const fPr = item.pr !== void 0 ? Number(item.pr) : Number(fPx);
          const fPb = item.pb !== void 0 ? Number(item.pb) : Number(fPy);
          const fPl = item.pl !== void 0 ? Number(item.pl) : Number(fPx);
          ...
          const fGrow = item.grow !== void 0 ? Number(item.grow) : null;
          const fPosition = item.position || "auto";
          const fAbsoluteX = item.x !== void 0 ? Number(item.x) : 0;
          const fAbsoluteY = item.y !== void 0 ? Number(item.y) : 0;
          const fOpacity = item.opacity !== void 0 ? Number(item.opacity) : null;
          const fVisible = item.visible === false || item.visible === "false" ? false : null;
          const fLocked = item.locked === true || item.locked === "true" ? true : null;
          const fTop = item.top !== void 0 ? item.top : null;
          const fRight = item.right !== void 0 ? item.right : null;
          const fBottom = item.bottom !== void 0 ? item.bottom : null;
          const fLeft = item.left !== void 0 ? item.left : null;
          const fCenterOffsetX = item.centerOffsetX !== void 0 ? Number(item.centerOffsetX) : null;
          const fCenterOffsetY = item.centerOffsetY !== void 0 ? Number(item.centerOffsetY) : null;
          const hasEdgeAttrs = fTop !== null || fRight !== null || fBottom !== null || fLeft !== null || fCenterOffsetX !== null || fCenterOffsetY !== null;
          ...
          const isNumeric = (v) => v !== void 0 && v !== "fill" && v !== "hug" && pctOf(v) === null;
          const numericW = isNumeric(item.w) ? item.w : isNumeric(item.width) ? item.width : void 0;
          const numericH = isNumeric(item.h) ? item.h : isNumeric(item.height) ? item.height : void 0;
          const hasWidth = numericW !== void 0;
          const hasHeight = numericH !== void 0;
          const fWidth = numericW !== void 0 ? numericW : 100;
          const fHeight = numericH !== void 0 ? numericH : 40;
          const alignMap = { start: "MIN", center: "CENTER", end: "MAX", stretch: "STRETCH" };
          const justifyMap = { start: "MIN", center: "CENTER", end: "MAX", between: "SPACE_BETWEEN" };
          ...
          const crossIsV = parentFlex === "row";
          const crossIsH = parentFlex === "col";
        ...
        ${parentVar}.appendChild(el${idx});`;
          ...
          const inner = item.innerRadius !== void 0 ? Math.max(0, Math.min(1, Number(item.innerRadius))) : 0;
          const hasArc = item.arc !== void 0 || item.arcStart !== void 0 || inner > 0;
          const startDeg = item.arcStart !== void 0 ? Number(item.arcStart) : 0;
          const sweepDeg = item.arc !== void 0 ? Number(item.arc) : 360;
        ...
        if (comp${idx}) {
          ...
          const slotP = item.p !== void 0 ? item.p : item.padding !== void 0 ? item.padding : null;
          const slotPx = item.px !== void 0 ? item.px : slotP !== null ? slotP : 0;
          const slotPy = item.py !== void 0 ? item.py : slotP !== null ? slotP : 0;
    ...
    generateCode(props, children, iconSvgMap = {}) {
      ...
      const isNumeric = (v) => v !== void 0 && v !== "fill" && v !== "hug";
      const numericWidth = isNumeric(rawWidth) ? rawWidth : void 0;
      const numericHeight = isNumeric(rawHeight) ? rawHeight : void 0;
      const hasExplicitWidth = numericWidth !== void 0;
      const hasExplicitHeight = numericHeight !== void 0;
      const width = numericWidth !== void 0 ? numericWidth : 320;
      const height = numericHeight !== void 0 ? numericHeight : 200;
      const bg = props.bg || props.fill || null;
      const stroke = props.stroke || null;
      ...
      const align = props.items || props.align || "MIN";
      const justify = props.justify || "MIN";
      const useSmartPos = props.x === void 0;
    ...
    parseGradient(str) {
      ...
      const stopsCode = stops.map(
        (s) => `{position:${s.position},color:{r:${s.color.r.toFixed(4)},g:${s.color.g.toFixed(4)},b:${s.color.b.toFixed(4)},a:${s.color.a}}}`
      ).join(",");
    ...
    generateEffectsCode(props, elementVar) {
      ...
      if (props.noise !== void 0 && props.noise !== null && props.noise !== "false" && props.noise !== false) {
        ...
        const eff = {
          type: "NOISE",
          noiseType,
          density: props.noiseDensity !== void 0 ? Number(props.noiseDensity) : 0.4,
          noiseSize: props.noiseSize !== void 0 ? Number(props.noiseSize) : 1.5,
        ...
        if (noiseType === "DUOTONE") {
          ...
          eff.opacity = props.noiseOpacity !== void 0 ? Number(props.noiseOpacity) : 0.5;
        }
        effects.push({ _raw: eff });
      }
      if (props.texture !== void 0 && props.texture !== null && props.texture !== "false" && props.texture !== false) {
        effects.push({ _raw: {
          type: "TEXTURE",
          noiseSize: props.textureSize !== void 0 ? Number(props.textureSize) : 12,
          radius: props.textureRadius !== void 0 ? Number(props.textureRadius) : 30,
          clipToShape: !(props.textureClip === "false" || props.textureClip === false),
          visible: true
        } });
      }
      if (props.progressiveBlur !== void 0 && props.progressiveBlur !== null) {
        ...
        if (Number.isFinite(r) && r > 0) {
          ...
          effects.push({ _raw: {
            ...
            radius: r,
            startRadius: props.progressiveBlurStart !== void 0 ? Number(props.progressiveBlurStart) : 0,
            startOffset: o.s,
            endOffset: o.e,
            visible: true
          } });
      ...
      if (props.glass !== void 0 && props.glass !== null && props.glass !== "false" && props.glass !== false) {
        effects.push({ _raw: {
          type: "GLASS",
          visible: true,
          refraction: props.glassRefraction !== void 0 ? Number(props.glassRefraction) : 0.95,
          depth: props.glassDepth !== void 0 ? Number(props.glassDepth) : 50,
          radius: props.glassRadius !== void 0 ? Number(props.glassRadius) : 6,
          dispersion: props.glassDispersion !== void 0 ? Number(props.glassDispersion) : 0.4,
          lightIntensity: props.glassLight !== void 0 ? Number(props.glassLight) : 0.7,
          lightAngle: props.glassLightAngle !== void 0 ? Number(props.glassLightAngle) : 130
        } });
      }
    ...
    async createEllipse(options = {}) {
      ...
      (function() {
        ...
        ${x !== void 0 ? `ellipse.x = ${x};` : ""}
        ${y !== void 0 ? `ellipse.y = ${y};` : ""}
    ...
    async createInstance(componentId, x, y) {
      ...
      (function() {
        ...
        ${x !== void 0 ? `instance.x = ${x};` : ""}
        ${y !== void 0 ? `instance.y = ${y};` : ""}
...
${notes}`;
  ...
  function parseBatch(jsxArrayJson, iconMapJson, gap, vertical, collection) {
    ...
    const iconMap = iconMapJson ? JSON.parse(iconMapJson) : {};
    const c = clientWith(iconMap, collection || null);
    return c.parseJSXBatch(jsxArray, { gap, vertical });
  }
